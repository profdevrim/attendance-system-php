...
<?php 
	...
	while($rows_EMP = mysqli_fetch_array($query_EMP)){ 		
		...
		?>
						...
						<a class="quick-button span2" href='calendar.php?uid=<?php echo base64_encode($rows_EMP['username']); ?>'>
							<i class="icon-user"></i>
			...
			<div class="row-fluid ">		
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon file"></i><span class="break"></span>Attandence Report</h2>
						<div class="box-icon">
...
<?php
	if($_SESSION['role']  == 0){
		...
		inner join attendance on attandance_master.master_id = attendance.master_id 
		inner join employee on employee.username = attandance_master.emp where username='".$_SESSION['user_id']."'";
	}else{
		$SQL = "select name, entry_date, time__in, time__out, status from attandance_master 
		inner join attendance on attandance_master.master_id = attendance.master_id 
		inner join employee on employee.username = attandance_master.emp where 1=1 ";
	}
